...
%fileName := "savepoint.bmp" %savepoint
%fileName := "blobs3.bmp" %savepoint
...
function getName : string
    var lastSlash : int := 1
    if index(fetcharg(0),"\\") = 0 then
	result fetcharg(0)
    else
	for i : 1 .. length (fetcharg(0))
	    if fetcharg(0)(i) = "\\" then
		lastSlash := max(lastSlash, i)
	    end if
       end for
       result fetcharg(0) (lastSlash+1 .. *)
    end if
end getName
...
proc debug(text:string)
    ...
    if debugFile = -1 then
	open(debugFile, getName + "_" + fileName + "_DebugLog.txt", "w")
    end if
...
/*
function checkNoise3(x, y, thresh : int) : int
    ...
    if View.WhatDotColor(x+1,y) = black then
	score := score +1
    end if
    if View.WhatDotColor(x-1,y) = black then
	score := score +1
    end if
    if View.WhatDotColor(x,y+1) = black then
	score := score +1
    end if
    if View.WhatDotColor(x,y-1) = black then
	score := score +1
    end if
    if View.WhatDotColor(x+1,y+1) = black then
	score := score +1
    end if
    if View.WhatDotColor(x-1,y-1) = black then
	score := score +1
...
if picID = 0 then
    ...
    %remove noize with spread
    var spread : int := 1
    for j : 0 .. height
	for i : 0 .. width
	    if(View.WhatDotColor(i,j) = black) then
		Draw.FillBox(i-spread, j-spread, i+spread, j+spread, blue)
	    end if
	end for
    end for
    ...
    for j : 0 .. height
	for i : 0 .. width
	    if(View.WhatDotColor(i,j) = blue) then
		Draw.Dot(i, j, black)
	    end if
	end for
    end for
    ...
    for j : 0 .. height
	for i : 0 .. width
	    Draw.Dot(i, j, checkNoise3(i, j, 2))
	end for
    end for
    */
    %break
    ...
    %implementation of blob finding using turing's fill
    for j : 0 .. height
	for i : 0 .. width
	    if(View.WhatDotColor(i,j) = black) then
		Draw.Fill(i, j, brightblue, white)
